...
class ForumService
{
    ...
    public function isPostRated($postid, $userid, $rating)
    {
        ...
        $this->db->query(
            "SELECT * from vote_post WHERE FK_UserID = ? AND FK_PostID = ? AND Vote = ?",
            $userid,
            $postid,
            $rating
        );
    ...
    public function toggleLike($postid, $userid)
    {
        if (ForumService::$instance->isPostRated($postid, $userid, 1)) {
            $this->db->query(
                "DELETE FROM vote_post WHERE FK_UserID = ? AND FK_PostID = ?",
                $userid,
                $postid
            );
        } else {
            $this->db->query(
                "REPLACE INTO vote_post (FK_UserID, FK_PostID, Vote) VALUES (?, ?, 1)",
                $userid,
                $postid
            );
        }
    ...
    public function toggleDislike($postid, $userid)
    {
        if (ForumService::$instance->isPostRated($postid, $userid, 0)) {
            ...
            $this->db->query(
                "REPLACE INTO vote_post (FK_UserID, FK_PostID, Vote) VALUES (?, ?, 0)",
                $userid,
                $postid
            );
        }
